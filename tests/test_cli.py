...
def test_agree_with_mock(tmp_path):
    """End-to-end CLI agree with mocked litellm."""
    # Create dataset JSONL
    dataset_path = tmp_path / "samples.jsonl"
    dataset_path.write_text(
        json.dumps({"id": "1", "input": "What is 2+2?", "output": "4"})
        + "\n"
        + json.dumps({"id": "2", "input": "What is 3+3?", "output": "6"})
        + "\n"
    )
    ...
    labels_path.write_text(
        json.dumps({"sample_id": "1", "scores": {"correctness": 5}})
        + "\n"
        + json.dumps({"sample_id": "2", "scores": {"correctness": 5}})
        + "\n"
    )
    ...
    assert result.exit_code == 0, f"CLI failed: {result.output}"
    assert "Agreement: Judge vs. Human Labels" in result.output
